...
# Task: Install Laravel Fortify and Build Blade-Based Authentication Flow

## Objective

Install and configure Laravel Fortify in the Laravel application, then create a complete Blade-based authentication flow that interacts with Laravel Fortify.
...
Install Laravel Fortify and configure it as the authentication backend for the application.

The implementation should include:

- Install Laravel Fortify via Composer.
- Publish Fortify configuration and service provider files.
- Register the Fortify service provider if required.
- Run the required migrations.
- Enable the required Fortify features:
  - Login
  - Registration
  - Logout
  - Password confirmation, only if needed
  - Password reset, only if needed

Fortify should handle the authentication logic, while all user-facing pages should be built manually using Blade templates.

---

## 2. Create a Shared Application Layout

Create a common Blade layout that will be used by the main pages of the application.
...
- Place for page-specific content using Blade sections or components.

All created pages should extend this layout.

---

## 3. Create a Common Header

Create a reusable header that appears at the top of the application.

The header should include:

- Application name or logo on the left.
- Navigation/menu area.
- Authentication controls on the right.

The header must behave differently depending on whether the user is authenticated or not.

---

## 4. Header Behavior for Guest Users

If the user is not authenticated:

- Display a **Login** button on the right side of the header.
- When the user clicks the **Login** button, they should be redirected to the Login page.
- The Login page should use the route handled by Laravel Fortify.

### Expected flow

1. User opens the application.
2. User is not authenticated.
3. Header shows the **Login** button.
4. User clicks **Login**.
5. User is redirected to the Login page.

---

## 5. Header Behavior for Authenticated Users

If the user is authenticated:

- Display the user’s short name or display name on the right side of the header.
- When the user clicks the name, a dropdown menu should open.
- The dropdown menu should contain a **Logout** button.
- Clicking **Logout** should submit a POST request to the Laravel Fortify logout route.
- After logout, the user should be redirected according to the application or Fortify configuration.

### Expected flow

1. User logs in successfully.
2. User is redirected to the application.
3. Header shows the authenticated user’s short name.
4. User clicks the name.
5. Dropdown menu opens.
6. User clicks **Logout**.
7. User is logged out.
8. Header changes back to the guest state and shows the **Login** button.

---

## 6. Create Login Page

Create a custom Blade Login page that works with Laravel Fortify.
...
- **Registration** button or link.

The Login form should:

- Send a POST request to the Laravel Fortify login route.
...
4. Laravel Fortify validates the credentials.
5. If credentials are invalid, validation errors are shown.
6. If credentials are valid, the user is authenticated.
7. User is redirected to the intended page, dashboard, or home page.

---

## 7. Add Registration Link on Login Form

On the Login page, add a **Registration** button or link.

The button should:

- Redirect the user to the Registration page.
- Be clearly visible near the Login form.
- Use the route connected to Laravel Fortify registration.
...
Create a custom Blade Registration page that works with Laravel Fortify.
...
- Send a POST request to the Laravel Fortify registration route.
- Include CSRF protection.
- Display validation errors returned by Laravel.
- Create a new user through Laravel Fortify.
- Authenticate the user after successful registration, if this behavior is enabled by Fortify.
...
3. User submits the form.
4. Laravel Fortify validates the submitted data.
5. If validation fails, errors are displayed.
6. If validation succeeds, a new user account is created.
7. User is redirected according to Fortify or application configuration.

---

## 9. Create a Common Menu

Create a shared menu inside the main layout or header.

The menu should:

- Be available across all pages using the shared layout.
- Contain common navigation links.
- Respect the authentication state where needed.
- Be easy to extend in the future.

Example menu items:

- Home
- Dashboard or Profile, only for authenticated users if needed
- Login, only for guest users

---

## 10. Routes and Fortify Integration

Configure Laravel Fortify to use custom Blade views.

The following Fortify views should be registered:

- Login view
- Registration view

Use Laravel named routes where possible:

- `login`
- `register`
- `logout`

The logout action must be performed using a POST request, not a GET request.

---

## 11. Authentication State Handling in Blade

Use Blade authentication directives to conditionally render UI elements.

Required conditions:

- Use `@guest` to show the Login button for unauthenticated users.
- Use `@auth` to show the authenticated user dropdown.
- Load the authenticated user name from `auth()->user()` or the `Auth` facade.

The short name can be generated from the user’s full name, for example:
...
Both Login and Registration forms should display validation errors.

Requirements:

- Show field-level validation errors where possible.
- Preserve old input values where appropriate.
- Do not preserve password fields after failed validation.
- Display a general error message for invalid login credentials if returned by Fortify.

---

## 13. Expected Pages and Components

The following Blade pages or components should be created:

- Main layout
- Header partial or component
- Menu partial or component
- Login page
- Registration page
- Optional home or dashboard page for testing authentication state
...
- Laravel Fortify is installed and configured.
- Custom Blade Login page is connected to Fortify.
- Custom Blade Registration page is connected to Fortify.
- Guest users see the Login button in the header.
- Authenticated users see their short name in the header.
- Clicking the authenticated user’s name opens a dropdown menu.
- The dropdown menu contains a working Logout button.
- Logout is performed through a POST request.
- Shared layout is used by all created pages.
- Shared menu is visible across the application.
- Validation errors are displayed correctly on Login and Registration forms.
- Full authentication flow works from guest state to login, registration, authenticated state, and logout.
...
You may be wondering when it is appropriate to use Laravel Fortify. First, if you are using one of Laravel's [application starter kits](/docs/13.x/starter-kits), you do not need to install Laravel Fortify since all of Laravel's application starter kits use Fortify and already provide a full authentication implementation.
...
If you choose to manually interact with Laravel's authentication services instead of using Fortify, you may do so by following the documentation available in the [authentication](/docs/13.x/authentication), [password reset](/docs/13.x/passwords), and [email verification](/docs/13.x/verification) documentation.

<a name="laravel-fortify-and-laravel-sanctum"></a>
#### Laravel Fortify and Laravel Sanctum

Some developers become confused regarding the difference between [Laravel Sanctum](/docs/13.x/sanctum) and Laravel Fortify. Because the two packages solve two different but related problems, Laravel Fortify and Laravel Sanctum are not mutually exclusive or competing packages.
...
If the request was not successful, the user will be redirected back to the login screen and the validation errors will be available to you via the shared `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors). Or, in the case of an XHR request, the validation errors will be returned with the 422 HTTP response.
...
If the request was not successful, the user will be redirected back to the two-factor challenge screen and the validation errors will be available to you via the shared `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors). Or, in the case of an XHR request, the validation errors will be returned with a 422 HTTP response.
...
If the request was not successful, the user will be redirected back to the registration screen and the validation errors will be available to you via the shared `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors). Or, in the case of an XHR request, the validation errors will be returned with a 422 HTTP response.
...
The value of the `$status` session variable will match one of the translation strings defined within your application's `passwords` [language file](/docs/13.x/localization). If you would like to customize this value and have not published Laravel's language files, you may do so via the `lang:publish` Artisan command:
...
If the request was not successful, the user will be redirected back to the request password reset link screen and the validation errors will be available to you via the shared `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors). Or, in the case of an XHR request, the validation errors will be returned with a 422 HTTP response.
...
If the request was not successful, the user will be redirected back to the reset password screen and the validation errors will be available to you via the shared `$errors` [Blade template variable](/docs/13.x/validation#quick-displaying-the-validation-errors). Or, in the case of an XHR request, the validation errors will be returned with a 422 HTTP response.
